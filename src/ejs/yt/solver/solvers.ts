...
import { extract } from "./nsig.js";
import { setupNodes } from "./setup.js";
import { generateArrowFunction } from "../../utils.js";
...
export function modifyPlayer(program: ESTree.Program) {
  ...
  const unpackedBody: ESTree.Statement[] = [];
  for (const node of block.body) {
    if (node.type === "ExpressionStatement" && node.expression.type === "SequenceExpression") {
      ...
      unpackedBody.push(node);
    }
  }

  block.body = unpackedBody.filter((node: ESTree.Statement) => {
